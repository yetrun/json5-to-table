...
class Matrix {
  ...
  subview (rowStart = 0, colStart = 0, rowSpan, colSpan) {
    if (_.isNil(rowSpan) || rowSpan >= this.rows) {
      rowSpan = this.rows - rowStart
    }
    if (_.isNil(colSpan) || colSpan >= this.cols) {
...
const missingMethods = _.difference(matrixMethods, matrixViewMethods).filter(methodName => !methodName.startsWith('_'))
missingMethods.forEach(methodName => {
  ...
  if (!('value' in methodDescriptor) || !(_.isFunction(methodDescriptor.value))) {
